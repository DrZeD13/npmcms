			...
			<div class="table-responsive">														
				...
				if (isset($data['article_row']))
				{
					...
					<tr class="<?=$row['status']?>">
						...
						<td>
							<a href="<?=$row['edit']?>" title="Редактировать"><?=$row['company']?></a>
						</td>
						<td>
							<?if ($row['totalcomments'] > 0) {?>
							<a href="/adm/comments/?user_id=<?=$row['id']?>" title="<?=$row['totalcomments']?> комментариев"><?=$row['totalcomments']?></a>
							<?}else{?>
								<?=$row['totalcomments']?>
							<?}?>
						</td>						
						...
						<td>
							<? if (!empty($row['del'])) {?>
							<a class="delete" href="<?=$row['del']?>" onClick="return confirm ('Вы действительно хотите удалить данного пользователя?');" title="Удалить"><i class="fa fa-minus-circle"></i></a>
							<?}?>
						</td>
					</tr>
					<?}
