		...
		<div class="main-content">	
			<div class="row head-search">
				<div class="col-sm-12">
					...
					<div class="col-md-3">					
						<form action="/adm/<?=$data["table_name"]?>/" method="POST">
						<span class="lable">Категория:</span>
						<?=$data['catalogtree']?>	
					</form>												
					</div>
			...
			<div class="table-responsive">														
				...
				if (isset($data['article_row']))
				{
					...
					<tr class="<?=$row['status']?>">
						...
						<td>
							<a href="<?=$row['edit']?>"><?=$row['name']?></a>
						</td>
						<td>
							<?=$row['date']?>
						</td>
						<td>
							<a href="/adm/fields_item/?field_id=<?=$row['id']?>" title="Значения дополнительных полей"><i class="fa fa-cart-arrow-down"></i></a>
						</td>
						<td>
							<i class="fa fa-filter <?=$row['is_filter']?>"></i>
						</td>
						<td>
							<?=$row['order_index']?>
						</td>
						...
						<td>
							<? if (!empty($row['del'])) {?>
							<a class="delete" href="<?=$row['del']?>" onClick="return confirm ('Вы действительно хотите удалить данный баннер?');" title="Удалить"><i class="fa fa-minus-circle"></i></a>
							<?}?>
						</td>
					</tr>
					<?}
