			...
			<div class="table-responsive">														
				...
				if (isset($data['article_row']))
				{
					...
					<tr class="<?=$row['status']?>">
						...
						<td>
							<a href="<?=$row['edit']?>" title="Редактировать"><?=$row['username']?></a>
						</td>
						<td>
						...
						<td>
							<? if (!empty($row['del'])) {?>
							<a class="delete" href="<?=$row['del']?>" onClick="return confirm ('Вы действительно хотите удалить данного администратора?');" title="Удалить"><i class="fa fa-minus-circle"></i></a>
							<?}?>
						</td>
					</tr>
					<?}
				...
				{?>
					<tr><td colspan="5" align="center"><?=$data['empty_row']?></td></tr>
				<?}?>
			</tbody>
			</table>
			</div>
