		...
		<div class="main-content">	
			<div class="row head-search">
				<div class="col-sm-12">
					...
					<div class="col-md-3">					
						<span class="lable">Пользователь:</span>
						 <?=$data['getUserSelect']?>											
					</div>
					<div class="col-md-3">
						<br>
						<a class="btn btn-flat btn-danger" href="/adm/<?=$data["table_name"]?>/delnotactivate" onClick="return confirm ('Вы действительно хотите удалить все неактивные комментарии?');" title="Удалить"><i class="fa fa-minus-circle"></i> Удалить все неактивные</a>
					</div>
			...
			<div class="table-responsive">														
				...
				if (isset($data['row']))
				{
					...
					<tr class="<?=$row['status']?>">
						...
						<td>
							<a href="<?=$row['edit']?>" title="Редактировать"><?=$row['name']?> <?=$row['new']?></a>
						</td>
						<td>
							<?=$row['date']?>
						</td>
