...
<table border="0" width="100%">
	<tr>
		<td>
			<form action="/adm/forms/" method="GET">
		...
		<td align="right">
			<a href="/adm/forms/add" class="add"><i class="fa fa-plus-square"></i> Добавить</a>
		</td>
	</tr>
</table>
<table width="100%" class="table" cellspacing="0" cellpadding="0">
	...
	if (isset($data['article_row']))
	{
		...
		<tr class="<?=$row['status']?>">
			...
			<td>
				<a href="/adm/form_item/?parent_id=<?=$row['id']?>"><?=$row['title']?></a>
			</td>
			<td>
				<?=$row['date']?>
			</td>
			...
			<td>
				<? if (!empty($row['del'])) {?>
				<a class="delete" href="<?=$row['del']?>" onClick="return confirm ('Вы действительно хотите удалить данную галерею?');" title="Удалить"><i class="fa fa-minus-circle"></i></a>
				<?}?>
			</td>
		</tr>
		<?}
