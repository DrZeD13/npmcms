...
<КоммерческаяИнформация xmlns="urn:1C.ru:commerceml_2" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ВерсияСхемы="2.07" ДатаФормирования="'.date("Y-m-dTH:i:s").'">';
	...
	function XMLCatalogTreeExport($parent = 0, $indent = "")
	{
		...
		if ($this->db->GetOne("select Count(*) from shops where is_active='1' and parent_id='".$parent."' limit 1", 0) > 0)
		{			
			$res .='
		'.$indent.'<Группы>';
			$sql = "select shop_id, name, parent_id from shops where is_active='1' and parent_id='".$parent."'";
			$result=$this->db->ExecuteSql($sql);
			while ($row = $this->db->FetchArray ($result))	
			{					
				$res .='
			'.$indent.'<Группа>
			'.$indent.'	<Ид>'.$row["shop_id"].'</Ид>
	...
	<Классификатор>
		...
		$res .='
		<Свойства>';
		$sql = "select field_id, name from fields where is_active='1' limit 5";
		$result=$this->db->ExecuteSql($sql);
		if ($result)
		{			
			while ($row = $this->db->FetchArray ($result))	
			...
			<Свойство>
				...
				<ВариантыЗначений>';
					$sql = "select field_item_id, value from fields_item where field_id='".$row["field_id"]."'";
					$result1=$this->db->ExecuteSql($sql);
					if ($result1)
					{			
						while ($row1 = $this->db->FetchArray ($result1))	
						{
							$res .='
					<Справочник>
						<ИдЗначения>'.$row1["field_item_id"].'</ИдЗначения>
	...
	<Ид>1</Ид>
		...
		$sql = "select shop.shop_id, shop.name, shop.parent_id, shop.count, shop.category, category.name as category_name, shop.short_text from shop join category on category.category_id = shop.category where shop.is_active='1' limit 5";
		$result=$this->db->ExecuteSql($sql);
		if ($result)
		{			
			while ($row = $this->db->FetchArray ($result))	
			...
			<Товар>
				...
				$sql = "SELECT field_id, field_item_id from fields_value WHERE parent_id = '".$row["shop_id"]."'";
				$result1 = $this->db->ExecuteSql($sql);
				if ($result1)
				{
					while ($row1 = $this->db->FetchArray ($result1))	
					...
					<ЗначенияСвойства>
						<Ид>'.$row1["field_id"].'</Ид>
						<Значение>'.$row1["field_item_id"].'</Значение>
					</ЗначенияСвойства>';						
					}
					$this->db->FreeResult ($result1);
				}
