...
class Model_Adm_Services extends Model 
{
	...
	var $orderType = array ("service_id"=>"", "name" =>"", "news_date"=>"", "is_active" => "", "url" => "");
	...
	function Insert()
	{
		...
		else
		{
			...
			$xsize = $this->db->GetSetting("XSizeSmallServicePhoto", 200);
            $ysize = $this->db->GetSetting("YSizeSmallServicePhoto", 200);
	...
	{				
		foreach ($this->bd_array as $key => $value)
		{
			switch ($value["type"])
			{
				...
				break;
				case "url":
					$data[$key] = $this->GetGP ("url", "");	
					if (($data[$key] == "") || (substr_count($data[$key], "/") == 0)) 
					{
						$data[$key] = validUrl($data[$key], $data["name"]);
						if ($data[$key])
						{
							$id = $this->GetID("id");
