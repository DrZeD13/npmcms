...
*/
class Model_Adm_Shops extends Model 
{

	var $table_name = 'shops';
	var $primary_key = 'shop_id';
	var $orderType = array ("shop_id"=>"", "name" =>"", "news_date"=>"", "order_index" => "", "is_active" => "");
	var $orderDefault = "order_index";
	var $orderDirDefault = "asc";
	var $path_img = "/media/shops/";
	var $rowsPerPage = 20;
	...
	{							
		...
		$navigation = ($parent_id == 0) ?  "" : "<div class='breadcrumb'><a href='".$this->siteUrl."adm/shops/?parent_id=0'>$title</a><span class='prow'> &raquo; </span>".$navigation."</div>";
	...
	function Edit()
	{
		...
		$menuarr = $this->get_array_catalog(false, $this->table_name, $this->primary_key);
		$menuarrtree = GetTreeFromArray($menuarr);
		$data = array (
			"name" => $this->dec($row["name"]),
			...
			'table_name' => $this->table_name,
			"action" => "update",
			"parents" => getMenusSelect($menuarrtree, $row['parent_id'], $row[$this->primary_key], $this->primary_key),
			"editor" => $this->editor(),
			"update" => array (
		...
		if ($filename != "")
		{
			...
			$link = "/adm/".$this->table_name."/del_img?id=".$id."&filename=filename";
	...
	{		
		...
		$data["url_error"] = "";
		$data["action"] = "insert";			
		$data["parents"] = getMenusSelect ($this->menuarrtree, $this->GetSession("PARENTSHOPID", 0), 0, $this->primary_key);
	...
	function Insert()
	{
		...
		{			
			...
			$xsize = $this->db->GetSetting("XSizeSmallShop", 200);
            $ysize = $this->db->GetSetting("YSizeSmallShop", 200);			
	...
	function Edit_error($edit = "update")
	{
		...
		{						
			$data["main_title"] = "Добавление пункта";
			$data["action"] = "insert";				
			$data["parents"] = $this->getMenus ($this->GetSession("PARENTSHOPID", 0), $id);
		}
		return $data;
	}
