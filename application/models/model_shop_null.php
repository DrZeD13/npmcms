...
// menu tree получить
		...
		else
		{
			...
			// проверяем количество потомков у каталога и если они есть будем обрабатывать шаблоном каталога $flag=false
			$sql = "SELECT Count(*) FROM ".$this->table_name." WHERE parent_id = '0'";
			$total = $this->db->GetOne($sql);
			if ($total == 0)
			{
				$flag_tamplate=true;
	...
	function Get_Filter($catalog_id)
	{
		...
		{	
			...
			while ($row = $this->db->FetchArray ($result))
			{
				...
				$this->array_filter["Бренд"]['item'][$row['category_id']] = array (
					"count" => $row['count'],
					"id" => $row['category_id'],
					"name" => $row['name'],
					"active" => false,
		...
		{									
			while ($row = $this->db->FetchArray ($result))
			{
				...
				$this->array_filter[$row['name']]['item'][$row['field_item_id']] = array (
					"count" => $row['count'],
					...
					if (($result) and ($this->db->Num_Rows($result) > 0))
					{
						...
						{							
							...
							if ($key == $row['url'])		
							{
								...
								$array_field_count_new[$row['name']][$row['field_item_id']] = $row['count'];
							}
						}
						$this->db->FreeResult ($result);												
						/*echo "<br>test<br>";*/
...
// получить menu tree		
		...
		if (!empty($row["related"])){
			...
			$related = $this->db->GetOne("select related from shops where shop_id = '".$row["parent_id"]."'", "");
		}
		if (isset($related) && (!empty($related))){
			$data['related_title'] = "Сопутствующие товары";
			$sql = "Select shop_id, name, filename, parent_id, url, short_text, price, count, recomend From `shop` Where is_active='1' and shop_id in (".$related.") ORDER BY news_date desc Limit 4";
		}
		else{
			$data['related_title'] = "Другие товары";
			$sql = "Select shop_id, name, filename, parent_id, url, short_text, price, count, recomend From `shop` Where is_active='1' and parent_id = '".$row["parent_id"]."' and shop_id <> '".$id."' Order By RAND () Limit 4";
		}
		$result = $this->db->ExecuteSql($sql);
		while ($row = $this->db->FetchArray ($result)) 
		{
			if ($row['filename'] != "") {          
				$extension = substr($row['filename'], -3);
				$recipelastimg = $this->siteUrl."media/shop/".substr($row['filename'], 0, -4)."_small.".$extension;
			}
			else {
				$recipelastimg = $this->siteUrl."img/noimg.jpg";
			}
			$fullurl = GetLinkCat($this->menuarr, $row["parent_id"]);
			$recipelastlink = $this->siteUrl.SHOP_LINK."/".$fullurl.$row['url'];
			$data['popular'][] = array (
					"id" => $row['shop_id'],
					"title" => $this->dec($row['name']),
